...
router.post('/signup', function(req, res) {
	...
	if (!req.body.username || !req.body.password) {
        ...
        // save the user
		var test = console.log(db.findOne(newUser));
        /*if(!test){
			res.json({success: false, msg: 'User already Exists.', headers: responseHeader, query: responseBody, env: process.env.UNIQUE_KEY});
		}else{*/
			db.save(newUser);
			res.json({success: true, msg: 'Successful created new user.', headers: responseHeader, query: responseBody, env: process.env.UNIQUE_KEY, test: test});
		//}
    }
});
